...
const NeuroharmonyFrontend: React.FC = () => {
  ...
  // Add debugging for datasets state
  useEffect(() => {
    console.log("Datasets state updated:", datasets);
  }, [datasets]);
  ...
  const filteredDatasets = Array.isArray(datasets)
    ? datasets.filter(
        (dataset) =>
          activeFilter === "ALL" || dataset.dataType === activeFilter,
      )
    : [];
            ...
            <div key={dataset.id} className="rounded-lg border p-4 shadow-lg">
              ...
              <div className="mt-4 rounded-md bg-gray-50 p-3">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <p>
                    Access:{" "}
                    {dataset.metadata?.permissions?.isPublic
                      ? "Public"
                      : "Private"}
                  </p>
                </div>
