...
def get_contract(web3):
    try:
        ...
        # Get ResearchFunding contract config
        contract_config = config["ResearchFunding"]
...
def create_grant(title, description, amount, private_key):
    try:
        ...
        txn = contract.functions.createGrant(title, description, amount).build_transaction({
        ...
        print(f"Error in create_grant: {str(e)}")
        raise
